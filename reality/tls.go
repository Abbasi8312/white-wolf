...
func (c *AcceptAllConn) Close() error {
	c.Target.Close()
	return c.Conn.Close()
}

func (c *AcceptAllConn) LocalAddr() net.Addr                { return c.Conn.LocalAddr() }
func (c *AcceptAllConn) RemoteAddr() net.Addr               { return c.Conn.RemoteAddr() }
func (c *AcceptAllConn) SetDeadline(t time.Time) error      { return c.Conn.SetDeadline(t) }
func (c *AcceptAllConn) SetReadDeadline(t time.Time) error  { return c.Conn.SetReadDeadline(t) }
func (c *AcceptAllConn) SetWriteDeadline(t time.Time) error { return c.Conn.SetWriteDeadline(t) }
...
func Server(ctx context.Context, conn net.Conn, config *Config) (*Conn, error) {
	...
	go func() {
		for {
			...
			if config.AcceptAll {
				// Derive AuthKey so handshake() can sign the certificate for client verification (same as non-AcceptAll path).
				var peerPub []byte
				...
				if peerPub != nil {
					if hs.c.AuthKey, err = curve25519.X25519(config.PrivateKey, peerPub); err == nil {
						_, _ = hkdf.New(sha256.New, hs.c.AuthKey, hs.clientHello.random[:20], []byte("REALITY")).Read(hs.c.AuthKey)
					}
				}
				if mc, ok := hs.c.conn.(*MirrorConn); ok {
					hs.c.conn = &AcceptAllConn{Conn: mc.Conn, Target: mc.Target}
				}
				hs.acceptedByAcceptAll = true
				break
			}
			var peerPub []byte
			for _, keyShare := range hs.clientHello.keyShares {
	...
	f:
		...
		if hs.c.out.handshakeLen[0] == 0 { // if the target sent an incorrect Server Hello, or before that
			if hs.c.conn == conn || hs.acceptedByAcceptAll { // if we processed the Client Hello successfully but the target did not
				waitGroup.Add(1)
				go func() {
	...
	} else if hs.c.conn != conn && !hs.acceptedByAcceptAll {
